...
label_map = {label: num for num, label in enumerate(gestureCollector.actions)}
sequences, labels = [], []

# Preprocess data
for action in gestureCollector.actions:
    for sequence in np.array(
        os.listdir(os.path.join(gestureCollector.DATA_PATH, action))
    ).astype(int):
        window = []
        for frame in range(gestureCollector.sequence_length):
            res = np.load(
                os.path.join(
                    gestureCollector.DATA_PATH,
                    action,
                    str(sequence),
                    "{}.npy".format(frame),
                )
